...
func getContainer(context *cli.Context, name string) (*cState, error) {
	...
	stateFile := filepath.Join(absRoot, name, runvcli.StateJSON)
	...
	state, err := runvcli.LoadStateFile(absRoot, name)
	if err != nil {
	...
	if state.Status != status {
		runvcli.SaveStateFile(absRoot, name, state)
	}
...
func updateContainerStatus(root, container, status string) error {
	state, err := runvcli.LoadStateFile(root, container)
	if err != nil {
		return err
	}
	if state.Status != status {
		state.Status = status
		return runvcli.SaveStateFile(root, container, state)
	}
