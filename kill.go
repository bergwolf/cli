...
func getProcessList(context *cli.Context, container string) ([]runvcli.Process, error) {
	pl, err := runvcli.NewProcessList(context.GlobalString("root"), container)
	if err != nil {
		return nil, err
	}
	defer pl.Release()
	...
	var (
		alive  []runvcli.Process
		update bool
	)
	// check if every shim is still alive
	for _, p := range plist {
		if runvcli.ShimProcessAlive(p.Pid, p.CreateTime) {
