...
is a directory with a specification file named "` + runvcli.SpecConfig + `" and a root
filesystem.
...
func cmdCreateContainer(context *cli.Context, attach bool) error {
	...
	ocffile := filepath.Join(bundle, runvcli.SpecConfig)
	spec, err := runvcli.LoadSpec(ocffile)
	if err != nil {
	...
	var scState *runvcli.State
	var vm *hypervisor.Vm
	var lockFile *os.File
	if sharedContainer != "" {
		scState, err = runvcli.LoadStateFile(root, sharedContainer)
		if err != nil {
			return err
		}
		vm, lockFile, err = runvcli.GetSandbox(filepath.Join(context.GlobalString("root"), sharedContainer, "sandbox"))
		if err != nil {
			return err
		}
	} else {
		f, err := runvcli.SetupFactory(context, spec)
		if err != nil {
			return nil
		}
		cpu, mem := getContainerCPUMemory(context, spec)
		vm, lockFile, err = runvcli.CreateAndLockSandBox(context, f, spec, cpu, mem)
		if err != nil {
			return nil
		}
	}
	...
	options := runvcli.RunvOptions{Context: context, WithContainer: scState, Attach: attach}
	_, err = runvcli.CreateContainer(options, vm, container, bundle, root, spec)
	if err != nil {
...
func findSharedContainer(root, nsPath string) (container string, err error) {
	...
	if strings.Contains(nsPath, "/") {
		...
		for _, item := range list {
			if state, err := runvcli.LoadStateFile(absRoot, item.Name()); err == nil {
