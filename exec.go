...
following will output a list of processes running in the container:
	...
	Action: func(context *cli.Context) error {
		...
		cState, cSpec, err := runvcli.LoadStateAndSpec(root, container)
		if err != nil {
...
func getProcess(context *cli.Context, spec *specs.Spec) (*specs.Process, error) {
	if path := context.String("process"); path != "" {
		return runvcli.LoadProcessConfig(path)
	}
...
func runProcess(context *cli.Context, container string, cState *runvcli.State, config *specs.Process) (int, error) {
	pid := os.Getpid()
	process := fmt.Sprintf("p-%x", pid+0xabcdef) // uniq name

	options := runvcli.RunvOptions{Context: context, WithContainer: cState, Attach: !context.Bool("detach")}
	...
	shim, err := runvcli.AddProcess(options, vm, container, process, config)
	runvcli.PutSandbox(vm, lockFile)
	if err != nil {
		return -1, err
	}
